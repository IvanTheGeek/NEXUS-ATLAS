...
- [x] `atlas-slice-design-concept.html` — actor column label vertical centring confirmed correct via bounding rects (center_y matches between labels and grid row items at each swimlane); earlier visual concern was a JPEG compression artifact, not a real alignment bug
- [x] `atlas-slice-design-concept.html` — INTERFACE row: actor column shows page background at the INTERFACE row (no swimlane colour bleeds in); dark divider visually contained to slice cards only; gap between cards and the label column blends as page background
- [x] `atlas-slice-design-concept.html` — MOBILE/DESKTOP labels properly horizontally centred in their 34px sub-lane strip; DRIVER label left-aligned with other top-level lane labels (ADMIN, RestAPI, CLI, SYSTEM); fix was adding `min-width: 0` to the flex sub-lane box to allow it to shrink below its min-content width
- [x] `atlas-slice-design-concept.html` — Slice 01 COMMAND zone: solid blue `--cmd-header` banner with "COMMAND" label right-aligned, rounded top corners flush with card, badges removed; Slice 02 retains old badge layout — side-by-side comparison of clean banner vs. badge-heavy header
